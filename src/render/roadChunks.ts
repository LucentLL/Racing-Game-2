 ...
 * H1168 — the chunk-grid engine is generalized into createChunkLayer so
 * the elevated marking pass (render/elevMarkChunks.ts) reuses it; the
 * ground-road layer below is the first instance.
 *
 ...
 *
 * Differences from the terrain recipe, all driven by these layers being
 * TRANSPARENT sheets of high-contrast vector art (terrain is opaque
 * pixel art):
 *
 *  - BAKE SCALE 3 px/world-px (terrain bakes 1:1 and NN-upscales).
 *    Live strokes rasterize at the world zoom (~2.93 internal px per
 *    wpx on desktop), so a 1:1 bake blitted through the same transform
 *    would visibly soften lane stripes. Scale 3 ≈ the live raster
 *    density; the blit downsamples ~0.98× with smoothing ON (pixel A/B
 *    verified at ship time: worst mean 0.52/255).
 *
 ...
 *
 *  - INTEGER-DEVICE-PX dest rects: two abutting drawImages at
 *    fractional device coords each write partial coverage into the
 *    shared boundary pixel; src-over composes the partial writes into
 *    a visible hairline (caught by the H1167 pixel A/B). Snapping
 *    every chunk edge to integer device px makes rects abut exactly.
 *
 *  - CONTENT PROBE: chunks whose probe reports nothing store
 *    canvas:null — no allocation, no blit. Most of the map for both
 *    instances.
 *
 * Correctness note on culling: bakes call the layer's paint function
 * with the chunk centre and half-extent; the paint pipelines' internal
 * culls are generous (entry bbox at cullR×1.6, chunk lookahead +460
 * wpx — far wider than any road's painted half-width), so every
 * primitive that intersects the canvas is drawn and the canvas edge is
 * the exact clip.
 */
 ...
 *  whole never-baked column into a single frame — these bakes are
...
interface ChunkTile {
  /** null = chunk verified content-free (probe) — no alloc, no blit. */
  canvas: HTMLCanvasElement | null;
  baked: boolean;
  used: number;
}

export interface ChunkLayerStats {
  visible: number;
  freshBakes: number;
  prefetch: number;
  pool: number;
...
export interface ChunkLayerCfg {
  /** window.<statsKey> gets the live stats object (headless probes
   *  must read the GAME's module instance — the known HMR gotcha). */
  statsKey: string;
  /** World-rect content probe — false lets the chunk skip allocation
   *  entirely. Rect arrives pre-inflated by ROAD_HALFW_WPX (bboxes are
   *  centerline extents). */
  probe(minX: number, minY: number, maxX: number, maxY: number): boolean;
  /** Paint the layer's content in WORLD coordinates. The factory has
   *  already set the scale+translate transform for the chunk. */
  bake(g: CanvasRenderingContext2D, centerWX: number, centerWY: number, radiusWpx: number): void;
}

/** Bbox inflation for the content probe — must exceed the widest
...
export interface ChunkLayer {
  draw(ctx: CanvasRenderingContext2D, centerX: number, centerY: number, radius: number): void;
}

export function createChunkLayer(cfg: ChunkLayerCfg): ChunkLayer {
  const chunks = new Map<number, ChunkTile>();
  let frameStamp = 0;
  let lastEpoch = -1;
  const _stats: ChunkLayerStats = { visible: 0, freshBakes: 0, prefetch: 0, pool: 0, epochFlushes: 0 };

  const chunkId = (cx: number, cy: number): number => cy * 4096 + cx;

  function bakeChunk(ch: ChunkTile, cx: number, cy: number): void {
    ...
    cfg.bake(g, originWX + CANVAS_WPX / 2, originWY + CANVAS_WPX / 2, CANVAS_WPX / 2);
    g.setTransform(1, 0, 0, 1, 0, 0);
    ch.baked = true;
  }

  function draw(ctx: CanvasRenderingContext2D, centerX: number, centerY: number, radius: number): void {
    frameStamp++;
    ...
    // Seam-proof dest rects — see header (integer-device-px snapping).
    // The world ctx is a uniform scale+translate (no rotation), so
    // device x = a·wx + e, y = d·wy + f.
    ...
    for (let cy = minCY - 1; cy <= maxCY + 1; cy++) {
      for (let cx = minCX - 1; cx <= maxCX + 1; cx++) {
        ...
        if (!ch) {
          ...
          // Content probe BEFORE any canvas allocation: an empty chunk
          // stores canvas:null and costs nothing per frame.
    ...
    (window as unknown as Record<string, unknown>)[cfg.statsKey] = _stats;
  }

  return { draw };
}

// ---- H1167 instance: the ground-road layer ------------------------------

const groundLayer = createChunkLayer({
  statsKey: '__roadChunkStats',
  probe: anyGroundRoadIntersects,
  bake: (g, cwx, cwy, r) => drawBaselineRoads(g, cwx, cwy, r),
});
...
export function drawRoadChunks(
  ...
  groundLayer.draw(ctx, centerX, centerY, radius);
}
